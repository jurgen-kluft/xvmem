...
namespace xcore
{
    ...
    static const u16 INDEX_NIL = 0xffff;
    struct xlist_t
    {
    ...
    void* xalloc_fsa_large::v_allocate(u32 size, u32 alignment)
    {
        ...
        if (is_empty_list(m_block_used_list))
        {
            if (!is_empty_list(m_block_empty_list))
            {
                u16 const i     = pop_from_list(m_list_array, m_block_empty_list);
                xblock_t* block = create_block_at(this, i);
                add_to_list(m_list_array, m_block_used_list, i);
                return allocate_from(this, i, size);
            }
        }
        else
        {
            u16 const i     = m_block_used_list;
            xbinfo_t* binfo = get_binfo_at(this, i);
            void*     ptr   = allocate_from(this, i, size);
            if (is_block_full(binfo))
            {
                remove_from_list(m_list_array, m_block_used_list, i);
                add_to_list(m_list_array, m_block_full_list, i);
            }
            return ptr;
        }
        return nullptr;
    ...
    u32 xalloc_fsa_large::v_deallocate(void* ptr)
    {
        ...
        if (is_block_full(binfo))
        {
            ...
            remove_from_list(m_list_array, m_block_full_list, block_index);
            add_to_list(m_list_array, m_block_used_list, block_index);
        }
        else
        {
            size = deallocate_from(this, block_index, ptr);
            ...
            {
                remove_from_list(m_list_array, m_block_used_list, block_index);
                destroy_block_at(this, block_index);
                add_to_list(m_list_array, m_block_empty_list, block_index);
            }
        }
        return size;
    }
